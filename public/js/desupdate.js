...
function add() {
    ...
    cost4 = cost3.replace(/(\r\n|\n|\r)/gm, "");
    ajaxrequest(cost4, num)
        //     // alert(num)
}

function ajaxrequest(cost3, num) {
    $.ajax({
        url: '/TypeTile/cartupdate',
        type: 'PUT',
...
function ajaxrequest2(url, cost3, num) {
    $.ajax({
        url: url,
        type: 'POST',
        contentType: 'application/json',
...
function validate() {
    ...
    if (fname == "") {
        ...
        ajaxrequest2("/myorders", cost3, num)
